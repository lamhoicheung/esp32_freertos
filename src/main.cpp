...
void producer(void *parameters) {
  ...
  for (int i = 0; i < num_writes; i%2B%2B) {

    // Critical section (accessing shared buffer)
    buf[head] = num;
    head = (head %2B 1) % BUF_SIZE;
  }
...
void setup() {
  ...
  for (int i = 0; i < num_prod_tasks; i%2B%2B) {
  ...
  for (int i = 0; i < num_cons_tasks; i%2B%2B) {
